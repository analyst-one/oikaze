...
> **Configure Oikaze directly within your SCSS files**
>
...
>
> Use the `.get()` function to access your theme values and `.css-definitions` to define CSS properties in `:root`.
...
>
> Use the `.alpha` function to generate values with opacity. Use the `.rem()`, `.em()`, and `.percentage` functions to generate values in various units. Prefix a token name with `$` to access the value directly from the token map (same as referencing the SASS variable). Use `.media()` mixin to generate media queries.
